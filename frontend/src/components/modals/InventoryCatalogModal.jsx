...
const TITLES = {
  ...
  damage: 'Dies with Damage Report',
  new: 'New Dies',
};
...
function InventoryCatalogModal({
  ...
  // Filtrado y orden ascendente por Die Description
  const filtered = data
    .filter(row =>
      (row.die_description_text || '').toLowerCase().includes(filter.toLowerCase())
    )
    .sort((a, b) => (a.die_description_text || '').localeCompare(b.die_description_text || ''));
  ...
  // Función para imprimir los datos filtrados
  ...
  const handlePrint = () => {
    ...
    // CSS para impresión y paginación
    const style = `
      <style>
    ...
    for (let p = 0; p < totalPages; p++) {
      const pageRows = filtered.slice(p * rowsPerPage, (p + 1) * rowsPerPage).map(row => `
        <tr>
      ...
      tablePages += `
        <div${p > 0 ? ' class="page-break"' : ''}>
          <div class="header">
            ...
            <div class="header-title">${TITLES[type]}</div>
            <div class="header-total">TOTAL: ${filtered.length}</div>
          </div>
          <table>
            <thead>
              <tr>
    ...
    printWindow.document.write(`<!DOCTYPE html><html><head><title>Print Inventory</title>${style}</head><body>${tablePages}</body></html>`);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-4xl p-0 relative max-h-screen overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b">
          ...
          <div className="font-bold text-blue-900 text-lg">TOTAL: {filtered.length}</div>
...
export default InventoryCatalogModal;
